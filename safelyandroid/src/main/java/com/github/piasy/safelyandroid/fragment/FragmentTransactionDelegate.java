 ...
 *
 * Delegate to commit {@link android.app.FragmentTransaction} from {@link TransactionCommitter}
 * safely, if it's resumed, then commit it right now, otherwise, it will be committed when
 * {@link #onResumed()} is called
 */
public class FragmentTransactionDelegate {

    private final List<android.app.FragmentTransaction> mPendingTransactions = new ArrayList<>();

    /**
     * commit a {@link android.app.FragmentTransaction} from {@link TransactionCommitter} safely
     *
    ...
    public synchronized boolean safeCommit(@NonNull TransactionCommitter committer,
            @NonNull android.app.FragmentTransaction transaction) {
